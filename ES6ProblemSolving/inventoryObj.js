...
// একটি Warehouse System Object-ভিত্তিক Product Stock Manage করে। 
// Action Parameter অনুযায়ী keys/values/entries বের করে, Property Delete করে, অথবা Object Seal/Freeze করে।
// Input
...
const manageInventoryObject = (obj, action) => {
    const validActions = ["keys", "values", "entries", "seal", "freeze"]
    const deleteAction = action.startsWith("delete:")
    if (typeof obj !== "object" || obj === null || Array.isArray(obj)) { return `Invalid` }
    if (!validActions.includes(action) && !deleteAction) { return `Invalid` }
    // return typeof obj !== "object"|| obj === null || Array.isArray(obj)? `Invalid`: action
    // return !validActions.includes(action) && !deleteAction? `Invalid`: action
